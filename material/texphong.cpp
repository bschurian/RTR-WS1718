...
void TexturedPhongMaterial::apply(unsigned int light_pass)
{
    ...
    if(bump.use) {
        ...
        prog_->setUniformValue("bump.tex", unit); bump.tex->bind(unit++);
    ...
    if(displacement.use) {
        ...
        prog_->setUniformValue("displacement.tex", unit); displacement.tex->bind(unit++);
    }
    ...
    // qDebug() << "next tex unit after textured_phong =" << unit;
}
